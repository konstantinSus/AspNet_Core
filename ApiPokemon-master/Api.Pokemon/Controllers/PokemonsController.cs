...
namespace Api.Pokemon.Controllers
{
    ...
    public class PokemonsController : Controller
    {
        ...
        public async Task<Data.PokemonResponce> GetInfoAboutPokemons()
        {
            ...
            var cache = await casheProcess.GetResultFromCashe(DateTime.Today);
            if(cache != null)
            {
                responce.Pokemons = cache;
                return responce;
            }
            responce.Pokemons = await new PokemonBusiness(_pokemonRepository).GetAllInfo();
            if(responce != null)
            {
